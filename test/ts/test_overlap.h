...
TEST(AABox, box_tree) {
	...
	bool res = Overlap<vt, 3>::DetectBox(bbtree, bs);
...
TEST(AABox, box_tree_tree) {
	...
	// box set ---------------------------------
	typedef Float vt;
	auto bsur = ConstructFromGtsFile(FILES + "head.gts", 0.0);
...
TEST(AABox, tri_tri) {
	// tri and its box -------------------------
	typedef Float vt;
	auto psur = ConstructFromGtsFile(FILES + "tri1.gts", 0.0);
	auto pf1 = *(psur->begin_face());
	//psur->transfer(2,0,0);
	AABBox<vt, 3> box1(pf1.get());//pTri

	auto bsur = ConstructFromGtsFile(FILES + "tri2.gts", 0.0);
	auto pf2 = *(bsur->begin_face());
	AABBox<vt, 3> box2(pf2.get());//pTri

	bool res = Overlap<vt, 3>::DetectObj(box1, box2);
	Point<vt, 3> pa, pb;
	bool res2= Overlap<vt, 3>::DetectIsect(box1, box2, pa, pb);
	...
	pf2 = *(bsur->begin_face());
	box2.reset(pf2.get());
	res = Overlap<vt, 3>::DetectObj(box1, box2);
	res2= Overlap<vt, 3>::DetectIsect(box1, box2, pa, pb);
	std::cout<< "Point a" << std::endl;
	pa.show();
	std::cout<< "Point b" << std::endl;
	pb.show();
...
TEST(AABox, box_tree_tree_vector) {
	...
	bool res2 = Overlap<vt, 3>::DetectObj(bbtree1, bbtree2, vec1, vec2);
	std::cout<< "overlap obj :" << res2 << std::endl;
