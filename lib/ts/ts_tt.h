...
//		const Scalar r1[3], const Scalar p2[3], const Scalar q2[3],
//		const Scalar r2[3], bool & coplanar, Scalar source[3],
//		Scalar target[3]);
...
template<class Scalar>
int ccw_tri_tri_intersection_2d(const Scalar p1[2], const Scalar q1[2],
...
int TriTriIsect(                           //
	...
	for (typename Triangle<VALUE, 3>::size_type i = 0; i < 3; i++) {
		V0[i] = tri1.get_vertex(0, ToAix(i));
		V1[i] = tri1.get_vertex(1, ToAix(i));
		V2[i] = tri1.get_vertex(2, ToAix(i));

		U0[i] = tri2.get_vertex(0, ToAix(i));
		U1[i] = tri2.get_vertex(1, ToAix(i));
		U2[i] = tri2.get_vertex(2, ToAix(i));
	}
	return TriTriRaw::tri_tri_overlap_test_3d(V0, V1, V2, U0, U1, U2);
}
