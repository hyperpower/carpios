...
public:
	...
	typedef Node_<COO_VALUE, VALUE, DIM> Self;
	typedef Node_<COO_VALUE, VALUE, DIM> *pSelf;
...
protected:
	...
	void _traversal(pNode pn, std::function<Ret(pNode&, Args)> fun,
			Args &args) {
		if (pn == nullptr) {
			return;
		} else {
	...
	void _traversal(const_pNode& pn,
			std::function<void(const_pNode&)> fun) const {
		if (pn == nullptr) {
			return;
		} else {
			fun(pn);
	...
	template<class Ret, class Args>
	void _traversal(const_pNode pn, std::function<Ret(const_pNode, Args)> fun,
			Args &args) const {
		if (pn == nullptr) {
			return;
		} else {
...
public:
	...
	St which_child(const COO_VALUE &x,
		...
		St idx = 0;
		if (Dim == 3) {
			idx = IsInRange(this->cell->get(_M_, _Z_), z,
					this->cell->get(_C_, _Z_), _cc_) ? 0 : 1;
		}
		idx = idx << 1;
		if (Dim >= 2) {
			idx = idx
					+ (IsInRange(this->cell->get(_M_, _Y_), y,
							this->cell->get(_C_, _Y_), _cc_) ? 0 : 1);
		}
		idx = idx << 1;
		idx = idx
				+ (IsInRange(this->cell->get(_M_, _X_), x,
						this->cell->get(_C_, _X_), _cc_) ? 0 : 1);
		return idx;
	}
	...
	inline Direction out_common_direction(const Direction &d) const {
		...
		static const unsigned short COMMON_AXES[4][4] = { { 0, 2, 1, 0 },
				...
				{ 1, 0, 0, 2 },
				{ 0, 1, 2, 0 } };
		std::function<unsigned short(unsigned short)> to2bit =
				[](unsigned short num) {
					unsigned short z = (num & 4) >> 2;
					return ((num & 1) << 1) + z;
				};
		std::function<unsigned short(unsigned short)> to3bit =
				[](unsigned short num) {
...
public:
	...
	pNode get_neighbor_adj_fast(const pNode Current, Direction d) {
		...
		switch (d) {
		case _XM_: {
			res = Current->neighbor[0];
			break;
		}
		case _XP_: {
			res = Current->neighbor[1];
			break;
		}
		case _YM_: {
			ASSERT(Dim >= 2);
			res = Current->neighbor[2];
			break;
		}
		case _YP_: {
			ASSERT(Dim >= 2);
			res = Current->neighbor[3];
			break;
		}
		case _ZM_: {
			ASSERT(Dim >= 3);
			res = Current->neighbor[4];
			break;
		}
		case _ZP_: {
			ASSERT(Dim >= 3);
			res = Current->neighbor[5];
			break;
		}
		default:
			res = nullptr;
	...
	template<class Args>
	void traversal(std::function<void(const_pNode, Args)> fun,
			Args &args) const {
		this->_traversal(this, fun, args);
	}
	...
	vt cda(St i) const {
		if (this->data != nullptr) {
			...
			vt sum = 0.0;
			std::function<void(const_pNode, St)> fun =
					[&sum, &nullflag, &count](const_pNode pn, St i) {
						if(pn->is_leaf()) {
							if(pn->data != nullptr) {
	...
	vt cdva(St i) const { //cell volume weight average
		if (this->data != nullptr) {
			...
			vt sum = 0.0;
			std::function<void(const_pNode, St)> fun =
					[&sum, &nullflag](const_pNode pn, St i) {
						if(pn->is_leaf()) {
							if(pn->data != nullptr) {
								if(pn->data->has_center(i)) {
									nullflag = false;
	...
	void show(const std::string& name = "") const {
		...
		std::cout << "       min    " << "     max    " << "     d    "
				<< "     c    \n";
		std::cout << "x:" << p(_M_, _X_) << " " << p(_P_, _X_) << " " << d(_X_)
				<< " " << cp(_X_) << "\n";
		std::cout << "y:" << p(_M_, _Y_) << " " << p(_P_, _Y_) << " " << d(_Y_)
				<< " " << cp(_Y_) << "\n";
		if (Dim == 3) {
			std::cout << "z:" << p(_M_, _Z_) << " " << p(_P_, _Z_) << " "
					<< d(_Z_) << " " << cp(_Z_) << "\n";
		}
		std::cout << "DATA  show =========\n";
 ...
 */
template<typename COO_VALUE, typename VALUE, St DIM> int GetDataIdx(
		const Node_<COO_VALUE, VALUE, DIM> *pn) {
	ASSERT(pn != nullptr);
...
template<typename COO_VALUE, typename VALUE, St DIM> Node_<COO_VALUE, VALUE, DIM> *
...
template<typename COO_VALUE, typename VALUE, St DIM> const Node_<COO_VALUE,
		VALUE, DIM> *
...
FaceType GetFaceType(  //
		const Node_<COO_VALUE, VALUE, DIM>* p,     //main node
		const Node_<COO_VALUE, VALUE, DIM>* pn) {
	if (p == nullptr) {
		return _Null_;
	}
