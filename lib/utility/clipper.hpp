...
#else
typedef signed long long cInt; // any value in the range ± 9.2e+18, better ±3.0e+9
...
/*
 * coordinate transfer double to cInt64
 */
inline cInt GetCoe(double min, double max) {
	double absmin = min < 0 ? -min : min;
	double absmax = max < 0 ? -max : max;
	double max_ = absmin < absmax ? absmax : absmin;
	return 1.0e+9 / max_;
}
...
public:
	...
	bool Execute(ClipType clipType, Paths &solution, PolyFillType subjFillType =
			pftEvenOdd, PolyFillType clipFillType = pftEvenOdd);
	bool Execute(ClipType clipType, PolyTree &polytree,
			PolyFillType subjFillType = pftEvenOdd, PolyFillType clipFillType =
					pftEvenOdd);
