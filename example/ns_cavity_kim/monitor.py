...
def plot_pressure_one(strstep, strtime):
    ...
    # gerris compare
    gmat = get_gerris_result_p("./gerris_result", RE, 6, "v")
    gx   = np.array(_col(gmat, 0))
    gv   = np.array(_col(gmat, 1))
    plt.plot(gv * 0.5, gx, color="k") 
    gmat = get_gerris_result_p("./gerris_result", RE, 6, "h")
    gx   = np.array(_col(gmat, 0))
    gv   = np.array(_col(gmat, 1))
    plt.plot(gx, gv *  0.5, color="k") 
...
def show_last(dir):
    ...
    ls = []
    for f in lfu:
        ls.append([int(f[1]), f[0], f[1], f[2]])
    ls = sorted(ls, key=lambda x: x[0])
    last = ls[-1]
    plot_velocity_one(last[2],last[3])
    plot_pressure_one(last[2],last[3])
